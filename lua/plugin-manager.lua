...
local function ensure(spec)
    ...
    -- Optional post-install build step, e.g. { "some/plugin", build = "make" }.
    -- Not needed by anything in plugin-list.lua today (blink.cmp runs pure-Lua
    -- fuzzy matching, so no compiled binary is required) but kept here so a
    -- future plugin that DOES need a build step can just declare it.
    if is_new and type(spec) == "table" and spec.build then
        print("Running build step for " .. name .. "...")
        vim.fn.system(string.format("cd %s && %s", vim.fn.shellescape(path), spec.build))
    end
end
